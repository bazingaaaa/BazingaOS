...
PRIVATE void set_smap()
{
	...
	/*为cmd.tar分配smap*/
	int bit_off = INSTALL_START_SECT - sb.n_1st_sect + 1;/*位偏移*/
	int bit_off_in_byte = bit_off % 8;
	int sect_beg = bit_off / 8 / SECTOR_SIZE;/*起始扇区*/
	int byte_off_in_sect = bit_off / 8 % SECTOR_SIZE;
	int bit_left = INSTALL_NR_SECTS;

	RD_SECT(ROOT_DEV, 2 + sb.nr_imap_sects + sect_beg);
	while(bit_left > 0)
	{
		fsbuf[byte_off_in_sect] |= (1<<bit_off_in_byte);
		bit_left--;
		bit_off_in_byte++;
		if(bit_off_in_byte == 8)
		{
			bit_off_in_byte = 0;
			byte_off_in_sect++;
			if(byte_off_in_sect == SECTOR_SIZE)
			{
				byte_off_in_sect = 0;
				WR_SECT(ROOT_DEV, 2 + sb.nr_imap_sects + sect_beg);
				sect_beg++;
				RD_SECT(ROOT_DEV, 2 + sb.nr_imap_sects + sect_beg);
			}
		}
...
PRIVATE void set_inode_array()
{
	...
	/*cmd.tar文件*/
	pNode = (struct inode*)(fsbuf + (NR_CONSOLES + 1) * INODE_SIZE);
	pNode->i_mode = I_REGULAR;
	pNode->i_size = INSTALL_NR_SECTS * SECTOR_SIZE;
	pNode->i_start_sect = INSTALL_START_SECT;
	pNode->i_nr_sects = INSTALL_NR_SECTS;
